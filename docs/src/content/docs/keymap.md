...
| `Backspace` / `Del` | Delete before / after cursor         |
...
`y` lists the contact's phones, emails, addresses and URLs, numbered 1 to 9. The value is sent through the terminal with OSC 52, so it works over SSH and in tmux (with `set-clipboard on`). The status shows "copied" as soon as the sequence is sent, even if the terminal ignores it.
